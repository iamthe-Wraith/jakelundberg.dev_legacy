...
const genericEmailErrorMessage = 'An error occurred while sending your message. Please try again later.';

const sendEmail = async (subject: string, message: string) => {
  try {
    if (!subject || !message) throw new ApiError('Please provide a subject and message.', HttpStatus.BAD_REQUEST);
...
export const sendGetInTouchEmail = async ({ email, subject, message }: IGetInTouchEmailRequest)  => {
  try {
    ...
    if (!email) throw new ApiError('Please provide an email address.', HttpStatus.INVALID_ARG, 'email');
    if (!subject) throw new ApiError('Please provide a subject.', HttpStatus.INVALID_ARG, 'subject');
    if (!message) throw new ApiError('Please provide a message.', HttpStatus.INVALID_ARG, 'message');
