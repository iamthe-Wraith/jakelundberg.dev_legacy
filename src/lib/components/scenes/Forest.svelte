<script lang="ts" type="module">
	...
	const flameCount = 50;
	const flameColors = [0xefc909, 0xdd6802, 0xdd5202];
	...
	onMount(() => {
		...
		Promise.all([loadForest(), loadTorch()]).catch(onError);
	...
	function animateTorch() {
		...
		for (let i = 0; i < flames.length; i++) {
			...
			flame.mesh.geometry.scale(scaleDegradation, scaleDegradation, scaleDegradation);
